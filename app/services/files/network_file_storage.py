...
class NetworkFileStorage(IFile):
    ...
    def extract_name(self, file, type):    
        ...
        if type.file_type == FileTypeEnum.ORIGINAL.value:
    ...
    def get(self,id, type) -> tuple:                
        ...
        original_name, path, extension = self.extract_name(fetched_file, type)
    ...
    def save(self,name_file,file_data, new_format) -> int:        
        ...
        try:            
            conn.connect(self.smb_server_ip, 445)            
            conn.storeFile(self.smb_folder_name, "\\public\\original\\"+temporal_name+"."+temp_original_name[1],BytesIO(file_data))            
        except Exception as e:
           print(f"Detail error: {e}")

        conn.close()
        upload_file = File(original_name=name_file, created_at=func.now(), temporal_name = temporal_name, new_format = new_format)
